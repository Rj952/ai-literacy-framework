              ...
              <main id="main-content">
                      <section aria-labelledby="assess-heading" className="py-16 border-b border-slate-800">
                                <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                                            <h1 id="assess-heading" className="text-4xl md:text-5xl font-bold text-slate-100 mb-4">AI Literacy Self-Assessment</h1>
                                            <p className="text-xl text-slate-400 max-w-3xl">Rate your confidence across the five foundational content areas.</p>
                                </div>
                      </section>
                      ...
                      <section className="py-16">
                                <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                                            <div className="card p-6 mb-8">
                                                          <label htmlFor="resp-name" className="block text-sm font-medium text-slate-400 mb-2">Your Name (Optional)</label>
                                            ...
                                            <div className="card p-6 mb-8">
                                                          <h2 className="text-sm font-bold text-slate-500 uppercase mb-4">Rating Scale</h2>
                            ...
                            <div key={l.value} className="p-3 rounded-lg bg-slate-800/50 border border-slate-700/50 text-center">
                                                <div className="text-2xl font-bold text-blue-400">{l.value}</div>
                                                <div className="text-sm font-semibold text-slate-300">{l.label}</div>
                          ...
                          <fieldset key={area.id} className="card p-6">
                                            <legend className="sr-only">{area.assessmentQuestion}</legend>
                                            <div className="flex items-center gap-3 mb-4">
                                                                <span className="text-2xl">{area.icon}</span>
                                                                <div><span className="text-xs font-bold uppercase" style={{color:area.colorAccent}}>Area {area.id}</span><h3 className="text-lg font-bold text-slate-100">{area.title}</h3></div>
                                            </div>
                                            <p className="text-slate-400 text-sm mb-4">{area.assessmentQuestion}</p>
                                            ...
                                            <div className="mt-8 text-center">
                                              ...
                                              {!allAnswered && <p className="mt-3 text-sm text-slate-500">Please rate all 5 areas to see results.</p>}
                                            </div>
                                
                                  {submitted && (
                        <div ref={resultsRef} tabIndex={-1} aria-live="polite" className="mt-12 card p-8">
                                        <h2 className="text-2xl font-bold text-slate-100 mb-2">Your Results</h2>
                          {name && <p className="text-slate-400 mb-4">Assessment for: <strong className="text-slate-200">{name}</strong></p>}
                                        <div className="text-center py-6 mb-6 border-b border-slate-700">
                                                          <div className="text-5xl font-bold mb-2" style={{color:getLevel(averageScore).color}}>{averageScore.toFixed(1)}</div>
                                                          <div className="text-xl font-semibold" style={{color:getLevel(averageScore).color}}>{getLevel(averageScore).label}</div>
                                        </div>
                                        <div className="space-y-4">
                                          {CONTENT_AREAS.map(area => (
                                              <div key={area.id}>
                                                                    <div className="flex justify-between text-sm mb-1"><span className="text-slate-300">{area.icon} {area.title}</span><span className="font-bold" style={{color:area.colorAccent}}>{responses[area.id]}/4</span></div>
                                        ...
                                        <div className="mt-8 flex gap-4 justify-center">
                                                          ...
                                                          <button onClick={()=>{setResponses({});setSubmitted(false);}} className="btn-secondary">Retake Assessment</button>
                                        </div>
                        </div>
                                            )}
                                </div>
